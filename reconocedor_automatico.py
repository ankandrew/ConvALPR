...
def main_demo(cfg, demo=True, benchmark=True, save_vid=False):
    ...
    if not is_img:
        logger.info(
            f"El intervalo del reconocimiento para el video es de: {intervalo_reconocimiento}"
        )
    while True:
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            "--cfg",
            dest="cfg_file",
            help="Path del archivo de config, \
                            default: ./config.yaml",
            default="config.yaml",
        )
        parser.add_argument(
            ...
            dest="demo",
            action="store_true",
            help="En vez de guardar las patentes, mostrar las predicciones",
        )
        parser.add_argument(
            "--guardar_video",
            dest="save_video",
            action="store_true",
            help="Guardar video en ./alpr-result.avi",
        )
        parser.add_argument(
            "--benchmark",
            dest="bench",
